import {
  ...
  export class MatHangManagementDetailDialogComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
      ...
      this.check = this.item.IdMH ? false : true;
    }
    ...
    getTitle() {
      ...
      return this.translateService.instant("Chi tiết");
    }
    ...
    savecontinute() {
      ...
      this.item.IdMH === 0
        ? this.create1(model, "saveAndContinue")
        : this.update(model);
    }
    ...
    create1(item: MatHangModel, actionType: string) {
      this.isLoadingSubmit$.next(true);
      if (
        this.authService.currentUserValue.IsMasterAccount ||
        !this.authService.currentUserValue.IsMasterAccount
