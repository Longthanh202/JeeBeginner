...
export class PhanNhomTaiSanManagementListComponent
  ...
  ngOnInit(): void {
    ...
    const vi = this.phannhomtaisanManagementService
  ...
  import() {
    ...
    const saveMessage = this.translate.instant(saveMessageTranslateParam);
    const messageType = MessageType.Create;
    const dialogRef = this.dialog.open(PhanNhomTaiSanManagementImportDialogComponent, {
      data: { item: item },
      width: '900px',
      height: '300px',
      disableClose: true,
  ...
  delete(_item: PhanNhomTaiSanModel) {
    ...
    const _deleteMessageTrung = `Phân nhóm tài sản còn hiệu lực không được xóa`;
